...
import { AdvantageInterface } from "../interfaces/interfaces";
import { getBackground } from "../utils/utils";

export default function Advantages() {
  const arrayAdvantages: AdvantageInterface[] = [
    {
    ...
    <div className="pt-10 w-full relative ">
      ...
      <div className="w-full  container ">
        ...
        <div className="md:w-full grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-3 pt-10 md:pt-12 lg:pt-16 w-[90%] mx-auto">
          {arrayAdvantages.map((item, index) => {
            return (
              <div
                key={index}
                className={`${getBackground(
                  index
                )}  bg-cover rounded-lg lg:rounded-xl bg-no-repeat bg-center lg:hover:scale-110 2xl:hover:scale-125 hover:z-50 hover:cursor-pointer transition-all duration-500 ease-in`}
              >
                <div className="bg-black/70  h-full rounded-lg lg:rounded-xl">
                  <div className="text-white py-5 md:py-7 2xl:py-10 space-y-[18px]">
                    ...
                    <div>
                      <p className="font-light text-center w-[90%] mx-auto tracking-wide text-sm">
                        {item.desc}
                      </p>
                    </div>
                  </div>
